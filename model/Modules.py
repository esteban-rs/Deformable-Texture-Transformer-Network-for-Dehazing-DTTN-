...
class ResBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, stride=1, res_scale=1):
        super(ResBlock, self).__init__()
        self.res_scale = res_scale
        self.conv1     = conv3x3(in_channels, middle_channels, stride)
...
class SFE(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        for i in range(self.num_res_blocks):
            self.RBs.append(ResBlock(in_channels     = n_feats, 
                                     middle_channels = n_feats, 
